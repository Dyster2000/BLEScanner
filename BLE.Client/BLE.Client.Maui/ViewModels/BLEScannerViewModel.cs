...
namespace BLE.Client.Maui.ViewModels
{
    public class BLEScannerViewModel : BaseViewModel
    {
        ...
        static public BLEDeviceViewModel? CurrentDevice { get; private set; }
        ...
        private void AddOrUpdateDevice(IDevice device)
        {
            MainThread.BeginInvokeOnMainThread(() => {
                ...
                {
                    if (vm.Update(device))
                        DebugMessage($"Update Device: {vm}");
                }
                else
                {
                    vm = new BLEDeviceViewModel(device);
                    DebugMessage($"Add Device: {vm}");
                    BLEDevices.Add(vm);
                }
            });
        }
        ...
        public void Device_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (e.CurrentSelection.Count == 0)
                CurrentDevice = null;
            else
                CurrentDevice = e.CurrentSelection[0] as BLEDeviceViewModel;
        }

        public void Connect_Clicked(object sender, EventArgs e)
        {
            ...
            if (AppShell.MainNavBar.Items.Count == 0)
                AppShell.MainNavBar.CurrentItem = null;
            else
                AppShell.MainNavBar.CurrentItem = AppShell.MainNavBar.Items[1];
        }
